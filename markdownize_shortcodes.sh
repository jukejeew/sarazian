...
short_dir="layouts/shortcodes"
mkdir -p "$short_dir"

# === เขียน shortcodes แบบ Markdown ===
cat > "$short_dir/about.md" <<'MD'
...
cat > "$short_dir/footer-note.md" <<'MD'
...
# === แก้ content ให้เรียกด้วย {{% %}} แทน {{< >}} ===
for f in content/about/_index.md content/posts/*.md; do
  [ -f "$f" ] || continue
  sed -i 's/{{</{{%/g; s/>}}/%}}/g' "$f"
  echo "updated -> $f"
done

echo "✅ เสร็จแล้ว! รัน: hugo server -D"
